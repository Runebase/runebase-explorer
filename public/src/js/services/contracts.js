...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(RunebaseCoreLib, Opcodes, Networks, Constants) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidRunebaseAddress: function (address) {
                return RunebaseCoreLib.Address.isValid(address, Constants.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = RunebaseCoreLib.crypto.Hash.sha256sha256(new RunebaseCoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return RunebaseCoreLib.encoding.Base58.encode(new RunebaseCoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = RunebaseCoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new RunebaseCoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = RunebaseCoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new RunebaseCoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = RunebaseCoreLib.crypto.Hash.sha256ripemd160(new RunebaseCoreLib.deps.Buffer(addr, 'hex'));
	...
	.factory('ERC20ContractInfo', function($resource, $window) {
		...
		return $resource($window.apiPrefix + '/rrc20/:contractAddress',
            {
                address: '@address'
            },
		{
